...
./gradlew :day-time-resolver:clean -console plain || exit
./gradlew :greeter:clean -console plain || exit

./gradlew :day-time-resolver:test :day-time-resolver:installDist -console plain || exit
./gradlew :greeter:test :greeter:installDist -console plain || exit

docker-compose -f docker-compose-with_jacoco_agent.yml up --build --force-recreate -d || exit

# wait for services to become available
sleep 30

curl http://localhost:8070/greet/Gipsy.Joe || exit
